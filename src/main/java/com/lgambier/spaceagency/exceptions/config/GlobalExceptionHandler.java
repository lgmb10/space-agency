...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<GlobalErrorResponse> handleMethodArgumentNotValidException(
        ...
        GlobalErrorResponse error = new GlobalErrorResponse(HttpStatus.BAD_REQUEST.value(), message,
    ...
    public ResponseEntity<GlobalErrorResponse> handleValidationException(ConstraintViolationException exc) {
         ...
         * */
        Set<String> constraintAnnotationTypes = exc
                                                        .getConstraintViolations()
                                                        .stream()
                                                        .map(v -> v
        ...
        Set<String> conflictValidators = Arrays
                                                 .stream(ConflictValidators.values())
                                                 .map(Enum::name)
                                                 .collect(Collectors.toSet());

        HttpStatus status = constraintAnnotationTypes
                                    .stream()
                                    .anyMatch(
                                            conflictValidators::contains) ? HttpStatus.CONFLICT : HttpStatus.BAD_REQUEST;
    ...
    public ResponseEntity<GlobalErrorResponse> handleDataIntegrityViolation(DataIntegrityViolationException exc) {
        ...
        GlobalErrorResponse error = new GlobalErrorResponse(HttpStatus.CONFLICT.value(), exc
